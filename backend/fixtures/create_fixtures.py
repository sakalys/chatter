...
from app.services.chat import user_code
...
async def create_example_conversation(db: AsyncSession, user: User, tool: MCPTool) -> None:
    ...
    conversation = Conversation(
        id=uuid.UUID("a4a929da-d636-434d-ad5e-35689d31eebb"), user=user, title="Example Conversation"
    )
    db.add(conversation)
    await db.commit()
    ...
    messages_data = [
        ...
        {
            "content": "Fixtures are used to populate a database with test data. In this project, we use SQLAlchemy.",
            "role": "assistant",
        },
        {
